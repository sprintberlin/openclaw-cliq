...
- **Portable message presentation (interactive buttons/cards).** The SDK's portable interactive surface is `MessagePresentation` (`{ title?, tone?, blocks: [...] }`) with block types `text` / `context` / `divider` / `buttons` / `select`; buttons carry `{ label, action?: {type:"command"|"callback", ...}, value?, url?, style?, disabled? }`. Two distinct capability surfaces gate it, and they are NOT interchangeable:
  ...
  The `MessagePresentation`/`MessagePresentationButton`/`ChannelPresentationCapabilities` types are NOT re-exported from any public `openclaw/plugin-sdk/*` subpath — they live only in the internal `payload-BHJeg3MX.d.ts` / `outbound.types-B7fkjz65.d.ts`. A plugin channel defines local structural types matching the portable shapes (a `{ title?, tone?, blocks: PortableBlock[] }` is structurally compatible) rather than importing internals.
  The full outbound rendering path is `ChannelOutboundAdapter.presentationCapabilities` + `renderPresentation({payload, presentation, ctx})` + `sendPayload(ctx)` (all on the `outbound.base` of `createChatChannelPlugin`, since it `Omit`s only `sendText`/`sendMedia`/`sendPoll`). When `renderPresentation` is absent, the runtime falls back to `renderMessagePresentationFallbackText` (degraded plain-text rendering) — so a channel can ship the explicit `message(action=send, buttons=[...])` tool-call path (via `CliqClient.sendCard`) first and add the reply-side `renderPresentation` path later without breaking. `adaptMessagePresentationForChannel({presentation, capabilities})` (exported from `openclaw/plugin-sdk/channel-runtime`) is what the runtime uses to truncate/split presentation blocks against the channel's declared `limits.actions` before calling `renderPresentation`.
