...
- **Required runtime OAuth scope `ZohoCliq.Chats.UPDATE` for native v3 chat typing.** Operators must re-consent the Self Client including this scope and regenerate the refresh token. Native typing is `POST /api/v3/chats/{chat_id}/activities` with body `{"action":"typing"}` (success is empty HTTP 204). Previously consented tokens without this scope fail typing with `oauthtoken_scope_invalid`. Implementation of `sendTyping` remains a follow-up (issue #175).
