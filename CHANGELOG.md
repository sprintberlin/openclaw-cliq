...
- **Block-streaming previews now grow during generation, even with `thinking.animate: "off"` (issue #185).** OpenClaw's buffered `deliver()` only flushes coalesced blocks, so a long turn could sit on the static placeholder until one final edit. The inbound path now also forwards `onPartialReply` snapshots into that same Cliq message when Core emits them. Independent of the thinking animator (issue #184). This is plugin-side wiring, not a verified default UX: on OpenClaw `2026.8.1-beta.3` with openai-completions `thinking`, Core does not emit those snapshots (issue #195 / openclaw/openclaw#132615), and the 2026-08-29 Mara DM stayed on the placeholder until one final edit (issue #194).
