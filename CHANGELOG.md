...
- Multipart recovery now accepts LF-only part framing in addition to canonical CRLF framing; a live replay showed Deluge-compatible payloads can reach the webhook with bare `\n` separators (#270).
- Deluge `body: payload` serialized as one multipart field per map entry (`handler`, `handlerSchema`, `message`, `user`, `chat`, `eventId`, `attachments`) is reconstructed even when the request falsely declares `Content-Type: application/json`; live text DMs at 16:53 on 2026-09-16 showed exactly that shape and were previously dropped as `parser_rejected` (#272).
- Deluge multipart bodies whose payload part has no `Content-Disposition` at all (JSON starts immediately after the boundary) are now parsed instead of falling through to form parsing and being dropped as `parser_rejected`; live text DMs at 09:19 on 2026-09-16 showed exactly that shape, with the boundary token surfacing as the first form key (#272).
- Deluge multipart bodies whose part headers run directly into the part content with no blank separator line are now parsed instead of falling through to form parsing and being dropped as `parser_rejected`; live attachment DMs (2026-09-16) showed exactly that framing, with the boundary token surfacing as the first form key (#272).
- Inbound file attachments are no longer dropped as `empty_body` when the Deluge payload carries a raw C0 control character inside a JSON string. Live attachment DMs (2026-09-19 18:22 and 18:50, `content-length=76847`/`76303`) delivered correctly quoted JSON that `JSON.parse` still rejected, because JSON forbids unescaped characters below U+0020 in a string and Cliq forwards them verbatim. Such characters are now escaped in string interiors only, so structural whitespace keeps its meaning; a body without them is left to the existing repairs. Verified live: the same MP3 upload that failed twice was accepted at 19:52 with no `empty_body` (#273).
- The multipart rejection now names the observed part list and the masked shape of the first text part, and persists the unparseable part to an owner-only file under the system temp directory (mode 600, path reported but content never logged or transmitted). The previous message was identical for every framing failure, which forced a guess-and-restart cycle per attempt (#273).
- Deluge `Map.toString()` payloads (bare keys joined by `=`, emitted by the attachment branch for nested maps) are parsed as a fallback when neither `JSON.parse` nor the quote/newline repair applies, and the multipart payload-part sniffer recognizes that shape so the part is identified at all (#273).
...
- Direct automation delivery now accepts Core-normalized Cliq targets such as `channel:<uniqueName>` and `user:<id>` in addition to full `cliq:` targets; this prevents `channel:` from being URL-encoded into the Zoho `channelsbyname` path and failing with `request_url_invalid`.
- Incoming Cliq voice notes and file attachments no longer stop at `inbound-media-no-fileid` when Zoho exposes only a generated filename. Attachment normalization now recognizes nested Zoho/Deluge file-id, MIME/name, and download-URL variants (including `params` wrappers and Files-API URLs); URL downloads are restricted to HTTPS on the configured Cliq v2 API origin. The generated Message Handler now forwards the original Deluge FILE objects as multipart data, while retaining JSON metadata and the history-lookup fallback, so the webhook can stage the actual bytes locally even when no API file id is present. Existing installations must repair/re-paste the Message Handler after upgrading; Mention remains attachment-free.
...
- **Doctor roundtrips now exercise multiline, quoted, entity-like text and name the shapes they cannot prove (issue #225).** A consented `openclaw cliq doctor --roundtrip` challenge now requires the complete copied multi-line body — nonce, reserved `example.invalid` email, fictional `+1 202-555-0100`, and quoted string — before it treats inbound delivery as observed; a nonce-bearing first line no longer passes. The report contains no challenge/message content, only the coverage result. Bot/handler inspection also lists literal `payload.put` field declarations for reply/quote and forwarded-message context: missing fields are explicit uncovered gaps, while declarations remain `declared_unexercised` because bot sends cannot synthesize native Cliq forwards or reply relationships. A real-client reply/quote plus forward remains the required live acceptance check.
...
- **Bot handlers now return their `eventId` instead of a bare `{}` (issue #231).** The generated Deluge script ended with `response = Map(); return response;`, so *every* row in the Zoho Bot execution log read `output: "{}"` — a delivered message, a rejected webhook, a transport failure and a handler that returned before `invokeUrl` were indistinguishable in the only log Zoho exposes. The 2026-09-05 forward incident had four executions, two agent turns and two missing turns, and `{}` could not explain the difference (issue #227). All three handlers (message, mention, welcome) now echo their generated `eventId`, so a Zoho execution row can be correlated with the gateway's own `evt:` identity. `openclaw cliq doctor` and the provisioning planner report a handler that does not echo it as `stale_script` with a confirmation-gated repair. The response carries the event id only — never the payload, the message text, the webhook secret or any token; the `invokeUrl` HTTP status is deliberately not captured, because a new Deluge symbol risks the permanent, non-retryable `execution_handler_update_failed` that the Mention Handler already demonstrated with `attachments`.

- **Rich, edited and unknown non-text Cliq messages now behave deterministically (issue #233).** An object-form message could carry its body in `content.text` / `description` rather than `text`, and an authenticated message whose `type` was neither `text` nor `file` returned `null` from the parser and disappeared with no turn. Body extraction now also reads `content.text`, `content.description` and `message.content` (attachments included), and an unknown type with no usable text, attachment or form content dispatches a deterministic `<cliq <type> message>` placeholder instead of being dropped. Only safe protocol labels reach the agent context — `CliqMessageType`, `CliqRichText`, `CliqRevision`, `CliqIsEdited` — with the type label normalized and length-bounded; card, location, contact and thumbnail payload objects are never retained. Rich markup is preserved as text rather than converted, because the `rte` shape has no stable webhook contract. No new Deluge symbol is introduced, so no handler faces the permanent `execution_handler_update_failed`. Edits remain correlated by real message id: Zoho supplies no distinct event identity for a revision, so a re-delivered edit is still seen as the same message by the dedupe layer.

### Fixed

- **Media replies no longer display the caption twice when reconciling the live placeholder (issue #237).** The reply caption is already carried by the new Cliq Files-API message. When an editable thinking placeholder is present, the placeholder now carries the visible text instead of duplicating it as the upload caption; non-editable/no-draft cases retain the caption on the attachment. This removes the duplicate text observed in the live end-to-end test on 2026-09-08 while preserving the reply text in every delivery mode.

- **File uploads now use Zoho's Files API instead of the message endpoints, which reject multipart outright (issue #242).** `sendMediaMessage` posted attachments as `multipart/form-data` to `POST /api/v2/bots/{bot}/message` (DM) and `POST /api/v2/channelsbyname/{name}/message` (channel). That contract was only ever held against unit-test mocks. Live probes against `cliq.zoho.com` (2026-09-08) show the real API refuses multipart on those endpoints altogether: with an `attachments` or `file` part it answers `404 operation_failed`, and even a multipart body carrying **no file at all** answers `400` — so the failure was never about the field name. Uploads now go to the dedicated endpoints from Zoho's published OpenAPI document (`files.yml`): `POST /api/v2/bots/{bot}/files` with the binary in `files` and the recipient in `user_id` (not `userids`), and `POST /api/v2/channelsbyname/{name}/files` with the sending bot in the `bot_unique_name` form field. The caption rides in `comments`, which must be a JSON **array** — a bare string is rejected with `input_json_invalid`. Both endpoints authorize with `ZohoCliq.Webhooks.CREATE`, so channel attachments no longer require a user-context refresh token. The success body is `{"user_ids":[…]}` and carries no message id, so `messageId` is normally `undefined`. Verified live: an XLSX reached a real Cliq DM with `HTTP 200`. Without this, the issue #237 / #238 delivery paths were wired correctly but still died at Zoho's 404.

- **`message(action=send)` no longer drops file attachments while reporting success (issue #238).** An agent send carrying `media` / `attachments` was resolved for text, buttons and cards only; the attachment params were unknown to the adapter, so the call fell through to the plain `sendMessage` text path and returned `Sent message to …`. Live evidence (2026-09-08): an XLSX send logged `[cliq] send: dm … textLen=90 api=v3` and the user received text without the file, while the agent had been told the delivery succeeded. A send with one loadable `media` / `attachments[].media` source now loads the bytes through `loadCliqMediaAttachment` (honouring the caller's `mediaReadFile` / `mediaAccess`) and posts them via the v2 multipart `sendMediaMessage`, with the message text as caption and an explicit `media: true` result detail. Attachment shapes Cliq cannot load (buffer/fileId sources), more than one attachment, and media combined with form/buttons/poll/slides/sections/thumbnail are now explicit failures instead of silent text-only sends; an unreadable source or a rejected upload also fails loudly. Text-only sends are unchanged.

- **A successful `message`-tool send no longer leaves an error bubble next to the delivered answer (issue #240).** `cleanupStrayPlaceholder` equated "no block consumed the thinking placeholder" with "the turn failed", but a turn whose visible output goes out through the `message` TOOL never touches the live-edit deliver — so a delivered reply and a dead turn were indistinguishable at cleanup time. Live evidence (2026-09-08): a tool-routed reply logged `send ok: status=200 … messageId=1788872980623` while the placeholder of the same turn was edited to the 33-character `⚠️ Couldn't process that message.`, leaving the user with both the answer and a failure notice. Successful tool sends are now reported through a turn-scoped observer (`observeCliqOutboundSends` / `notifyCliqToolSend`) and, when one targeted this conversation, the redundant placeholder is deleted instead of rewritten. Only TOOL-origin sends count: the plugin's own placeholder, progress-draft and reply sends must never mark a turn as answered, so genuinely failed or empty turns still show the notice and the #91 / #123 regression tests stay green.

- **A reply's `MEDIA:` attachment now reaches the chat instead of being rendered away (issue #237).** Core hands final-block media to the dispatcher's `deliver` as `mediaUrl` / `mediaUrls`, but the inbound wrapper forwarded only `text` and `channelData`, and the live-edit deliver declared `mediaUrl` in its payload type without ever reading it — so the plugin's working v2 multipart path (`sendMediaMessage`, locked by the issue #65 regression tests) was unreachable from a normal turn. Live evidence (2026-09-08): a reply carrying `MEDIA:…/gregor_sprint_crm_pruefung.xlsx` delivered the sentence without the file, and seven days of gateway logs contained **zero** `[cliq] send media` lines. Both fields are now forwarded and posted as attachments, with the reply text as the caption of the first file and overflow text following as plain messages. A Cliq attachment cannot be edited into an existing message, so the thinking placeholder / progress draft is reconciled exactly as the card path does it (edited to the caption, else deleted) instead of being left for the stray-placeholder failure notice. Media on a progress draft is skipped so the preview keeps its own lifecycle, and a media source that cannot be loaded or posted falls back to the text path — a lost attachment never swallows the answer.

- **`user:`-prefixed targets are accepted and bare numeric ids reach the DM they name (issue #239).** `parseCliqTarget` only recognized a direct target in the fully-prefixed `cliq:user:<id>` form. An agent-supplied `user:<id>` was rejected outright (`Unknown target "user:929484733" for Zoho Cliq`), and a bare `<id>` fell through to the documented group default — so a numeric Zoho user id was posted to `/api/v2/channelsbyname/<id>/message` and answered `404 request_url_invalid`, even though the advertised `targetResolver` hint (`<channelUniqueName|userId>`) explicitly invites a bare user id. Provider-less `user:` / `dm:` / `channel:` / `chat:` / `group:` kind prefixes are now parsed as explicit targets, and a bare **all-numeric** id resolves to a direct target (Zoho user ids are numeric; channel unique names never are, so no legitimate channel name is swallowed). Bare non-numeric ids keep the group default, and an unknown provider-less `foo:bar` prefix is still rejected rather than guessed.

- **Corrupt Deluge webhook bodies are repaired instead of dropped (issues #223 / #227).** Zoho's `payload.toString()` does not JSON-escape string values, so any message containing a double quote or line break arrived as structurally-complete but unparseable JSON and died at `JSON.parse`. Live evidence (2026-09-05): a forwarded business instruction arrived as an authenticated 1091-byte POST with the full original text — and was rejected as `empty_body`. Because the generated handler's grammar is known (`"handler":"…","message":"<free text>","user":…`), the webhook now re-escapes the message value at the last structural `","user":` boundary and re-parses; unrepairable bodies log a content-free syntax fingerprint (`shape: …`, words masked, punctuation preserved) in the `empty_body` skip line. A body that already parses is never touched, and the repair cannot force a value the re-parse rejects. See the new `src/inbound-deluge-repair.ts`.

- **Every acknowledged inbound that creates no agent turn now logs a reason at the default level (issue #232).** `POST /cliq/webhook` has many paths that answer `200` without dispatching — self-message protection, dedupe, mention gating, admission, pairing, welcome replays. Most logged at `debug` or not at all, so a default gateway/journald view showed no inbound line even though Zoho had posted successfully. During the 2026-09-05 forward incident that made a short Zoho handler execution with `output: {}` indistinguishable between "the POST never arrived" and "the POST arrived and was silently skipped" (issue #227). Each of those paths now emits exactly one `[cliq] inbound skipped: <reason> (message=… sender=… detail=…)` line at `warn`, drawn from a stable vocabulary — `empty_body`, `parser_rejected`, `self`, `duplicate`, `inflight`, `not_mentioned`, `not_admitted`, `pairing`, `pairing_action`, `welcome_skipped` — that doctor and log pipelines can match on. Identifiers and reason codes only; never message text, display names, email addresses, scripts, tokens, or the webhook secret (the self-message line reports the matched *field name*, not the matched value). Admission and dedupe policy are unchanged. See the new `src/inbound-outcome.ts`.

- **Parser rejects are no longer silent (issue #224).** `POST /cliq/webhook` logged nothing when `parseCliqWebhookPayload` returned `null`, making an unsupported message shape indistinguishable from "Zoho never called us". The reject path now logs the rejection reason (missing `user.id` vs. no usable body vs. textless forward) plus the payload's top-level key names only; never values, never secrets. It uses the shared `inbound skipped: parser_rejected` vocabulary above.

- **An unreadable webhook body no longer disappears (issues #224 / #232).** An authenticated POST whose body was empty or unparseable was the last fully silent drop on the inbound path, so "Zoho posted something we could not read" was indistinguishable from "Zoho never called us". It now logs `inbound skipped: empty_body` with the parse-failure reason and the content length — never the body.
...
- **Core progress drafts now render and finalize through Cliq's one-message live-edit transport (issue #209).** `streaming.mode: "progress"` converts Core-composed status headlines, plan checklists, and sanitized rolling lines once through `markdownToCliq`, reuses a text thinking placeholder or creates one editable draft, honors the existing edit throttle plus Core `flush`, and caps the temporary status at 5000 characters. Fitting finals replace that draft in place, long finals reuse it for the first chunk, and failed replacements still send the complete final while stale-draft deletion remains best-effort. Card-only finals, no-reply errors, and benign skips retire editable text drafts through the existing edit fallback. Unsupported v2 DM, v3 channel-post, and v3 thinking-card surfaces remain final-only.

- **Progress-mode turns now feed OpenClaw Core's compositor (issue #208).** `channels.cliq.streaming.mode: "progress"` instantiates `createChannelProgressDraftCompositor` and wires Core `onToolStart` / `onItemEvent` / `onPlanUpdate` / `onApprovalEvent` / `onCommandOutput` / `onPatchSummary` / `onNarrationUpdate` into one draft. `onPartialReply` stays on `"partial"` only, so answer snapshots cannot clobber progress before final delivery. Fast turns still honor Core's 1.5s start gate, default standalone tool-progress messages are suppressed while the draft owns them, and reasoning is forwarded only when the existing OpenClaw `reasoningDefault` / session `reasoningLevel` is `"stream"`. End-to-end gateway and isolated live acceptance remain in issue #210.
...
- **Cliq streaming config now uses OpenClaw Core's shared mode and progress schema (issue #207).** `channels.cliq.streaming.mode` accepts `"off"`, `"partial"`, `"block"`, and `"progress"`; `streaming.progress` accepts the Core compositor settings, and `minEditIntervalMs` still controls the v2 Cliq edit throttle. Existing unset config keeps today's effective `"partial"` answer-preview behavior. Legacy `streaming.preview: "on" | "off"` remains loadable as `"partial" | "off"`; explicit `mode` wins, and `openclaw doctor --fix` rewrites the legacy key at the root and per-account levels without exposing secrets.

- **README streaming docs now name both switches and the live model matrix (issue #205).** `channels.cliq.streaming.preview` is the Cliq one-message live-edit path and asks Core for block/partial delivery on that turn; `agents.defaults.blockStreamingDefault` is the fleet/global Core default. The working config is both on, `thinking.mode: "placeholder"`, `thinking.animate: "off"`, and a `refreshToken`. Live A/B on Mara (OpenClaw `2026.8.1-beta.3`, plugin `7ab02f9` / 0.1.10): `sprintcx/tier-1` + `thinking=medium` stayed `textLen=4` then one `edit textLen=4097`; `sprintcx/tier-2` grew `4 → 68 → 1269 → 2075 → 2878 → 3493 → 4187 → 4942`. The guarantee is same message plus in-place final; progressive intermediate growth only when the selected model emits intermediate text.
...
- **README streaming/thinking docs no longer oversell live growth (issue #194).** The operator-facing `thinking` / `streaming` entries are split into short subsections: default preview is block-streaming, not Telegram-style token growth; `onPartialReply` is not guaranteed on OpenClaw `2026.8.1-beta.3`; HTTP 204 is API acceptance only (the Cliq client may keep showing `⏳ …` until the chat is reopened); and `streaming.preview: "off"` / `thinking.mode: "off"` restore a normal message. The 2026-08-29 Mara DM (`textLen=4` → one final `textLen=117`) is the first documented live miss; #175/#185 never recorded a live Cliq round-trip. The issue #185 changelog line described plugin-side snapshot wiring, not a verified default UX.

- **Block-streaming live-edit no longer claims token-level growth on every OpenClaw `2026.8.1-beta.3` turn (issue #195).** The plugin still wires `onPartialReply` into the same Cliq draft. Some openai-completions thinking streams (live: `sprintcx/tier-1` + `thinking=medium`) stamp `openclawDelivery.textPhaseRequiresTerminal` and stay on the placeholder until one final edit ([openclaw/openclaw#132615](https://github.com/openclaw/openclaw/issues/132615)); others (live: `sprintcx/tier-2` + `thinking=medium`) grow in place. See issue #205 for the model matrix.
...
- **Block-streaming previews now grow during generation, even with `thinking.animate: "off"` (issue #185).** OpenClaw's buffered `deliver()` only flushes coalesced blocks, so a long turn could sit on the static placeholder until one final edit. The inbound path now also forwards `onPartialReply` snapshots into that same Cliq message when Core emits them. Independent of the thinking animator (issue #184). This is plugin-side wiring, not a promise that every model emits snapshots: `sprintcx/tier-1` + `thinking=medium` can still stay on the placeholder until one final edit (issue #195 / openclaw/openclaw#132615); `sprintcx/tier-2` grows in place (issue #205).
