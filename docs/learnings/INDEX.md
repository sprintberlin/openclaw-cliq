...
- [Channel setup `credentials` is how OpenClaw stores SecretRefs; a custom `finalize` prompt still writes literals](131-channel-setup-credentials-own-secretref-storage.md)  — files: src/setup-wizard.ts,src/setup-report.ts · apis: ChannelSetupWizard.credentials,runSingleChannelSecretStep,promptSecretRefForSetup,finalize
