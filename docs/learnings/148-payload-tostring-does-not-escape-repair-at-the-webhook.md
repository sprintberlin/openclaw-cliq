...
Generated handlers now avoid this by posting `body: payload` (issue #259). The webhook still repairs the known corruption for unrepaired handlers instead of dropping it. The generated handler's field vocabulary is fixed, but Deluge may vary insignificant whitespace and map order; the message value is re-escaped at the last parseable separator before a known generated field and the whole object is re-parsed. Rules that keep this safe:
...
The durable source fix is passing the Map to `invokeUrl body:` rather than hand-serializing with `toString()`. Manual Deluge escaping loops remain rejected: they would need new handler symbols across every install — the permanent `execution_handler_update_failed` risk — and would not help old handlers already deployed. The gateway-side repair therefore stays as a compatibility fallback, not as the primary contract.
