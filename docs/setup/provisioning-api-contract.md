...
The Message Handler script should forward `message`, `user`, and `chat` with `handler: "message"`. For attachments it should put a `getFileName()` list in the JSON metadata and send the original Deluge FILE objects to `/cliq/webhook` through `invokeUrl files:` together with a `stringPart` named `payload`; `payload.toString()` alone can collapse a voice note to its displayed filename and discard the bytes. The Mention Handler script should forward `message`, `user`, and `chat` with `handler: "mention"`, but must omit every attachment/multipart reference. Text-only Message and Mention deliveries POST raw JSON using `body: payload.toString()` and `Content-Type: application/json`; `parameters:` form-encodes the payload and is invalid for this webhook.
