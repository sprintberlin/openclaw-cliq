...
describe("renderCliqFormCards", () => {
  it("renders a single select field as one prompt card with a button per option", () => {
    ...
    expect(cards[0].buttons![1]).toMatchObject({ data: "priority: medium" });
    expect(cards[0].buttons![2]).toMatchObject({ data: "priority: low" });
  });
  ...
  it("renders string options (label = value)", () => {
    ...
    expect(cards[0].buttons).toHaveLength(3);
    expect(cards[0].buttons![0]).toMatchObject({ label: "prod", data: "env: prod" });
    expect(cards[0].buttons![2]).toMatchObject({ label: "dev", data: "env: dev" });
  });
  ...
  it("renders multiple select fields as separate prompt cards", () => {
    ...
    expect(cards[0].buttons![0].data).toBe("region: us");
    expect(cards[1].title).toBe("tier");
    expect(cards[1].buttons![0].data).toBe("tier: free");
  });
  ...
  it("clamps over-length button labels", () => {
    ...
    expect(cards[0].buttons![0].data).toBe("x: v");
  });
  ...
  it("button data uses field name (not label); card title uses field label", () => {
    ...
    expect(cards[0].buttons![0].data).toBe("priority_field: high");
