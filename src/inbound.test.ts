...
describe("dispatchCliqInbound — thinking placeholder (issue #47)", () => {
  ...
  it.each([
    ...
    "passes streaming.mode=$mode through to the dispatcher",
    async ({ blockStreaming, mode, disableBlockStreaming, expectPartial }) => {
      let replyOptions: {
        disableBlockStreaming?: boolean;
        onPartialReply?: (payload: { text?: string }) => unknown;
        onToolStart?: (payload: { name?: string }) => unknown;
      } | undefined;
      ...
      if (mode === "progress") {
        expect(typeof replyOptions?.onToolStart).toBe("function");
      } else {
  ...
  it("hands the thinking placeholder off to the streaming preview (one message, no duplicate)", async () => {
    const client = makeMockClient({ placeholderChatId: "chat-u1" });
    const parsed = parseCliqWebhookPayload(dmPayload());
    await dispatchCliqInbound({
      ...
      channel: {
        ...
        inbound: {
          run: async (params) => {
            ...
            await turn.delivery.deliver({ text: "first" });
            await turn.delivery.deliver({ text: "second" }, { final: true });
          },
        },
        pairing: {
          buildPairingReply: () => "",
  ...
  it("wires Core progress callbacks in progress mode without onPartialReply (issue #208)", async () => {
    const client = makeMockClient({ placeholderChatId: "chat-u1" });
    const parsed = parseCliqWebhookPayload(dmPayload());
    let replyOptions:
      | {
      ...
      channel: {
        ...
        inbound: {
          run: async (params) => {
            ...
            const turn = adapter.resolveTurn({}, {}, {}) as unknown as {
              replyOptions?: typeof replyOptions;
              delivery: {
                deliver: (
                  payload: { text?: string },
                  info?: { final?: boolean },
...
describe("dispatchCliqInbound — thinking placeholder cleanup on no reply", () => {
  ...
  it("ignores tiny onPartialReply snapshots so the thinking placeholder is preserved (issue #203)", async () => {
    const client = makeMockClient({ placeholderChatId: "chat-u1" });
    const parsed = parseCliqWebhookPayload(dmPayload());
    ...
    runtime.channel.inbound.run = async (params) => {
      ...
      const turn = adapter.resolveTurn({}, {}, {}) as unknown as {
        replyOptions: { onPartialReply?: (payload: { text?: string }) => Promise<void> | void };
        delivery: { deliver: (payload: { text?: string }, info?: { final?: boolean }) => Promise<void> };
      };
      // Send a 1-character snapshot (as observed live on Mara), then a 2-char snapshot.
      await turn.replyOptions.onPartialReply?.({ text: "1" });
      await turn.replyOptions.onPartialReply?.({ text: "12" });
      expect(client.edits).toHaveLength(0);
      // Then deliver the full final answer.
      await turn.delivery.deliver({ text: "The final answer with full content." }, { final: true });
    };
    ...
    expect(client.sends[0].text).toBe("⏳ …");
    expect(client.edits).toHaveLength(1);
    expect(client.edits[0].text).toBe("The final answer with full content.");
  });
