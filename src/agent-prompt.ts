...
export function resolveCliqMessageToolHints(): string[] {
  return [
    ...
    "- Cliq polls (v3 opt-in only, `apiVersion: \"v3\"`): `action=send` accepts `theme: \"poll\"` + `pollOptions` (array of 2–10 plain-text strings, ≤100 chars each) to post a voting card. Cliq counts votes natively — no callback to the bot, so poll options are NOT action buttons. The `message` text is the poll question (first line = card title). On v2 / unconfigured v3 the poll degrades to plain text.",
    "- Cliq supporting slides (v3 opt-in only, `apiVersion: \"v3\"`): `action=send` accepts a `slides` param (array of structured content blocks rendered alongside the card, compatible with all card themes). Each entry is `{ type, title?, ... }` where `type` is `table` (`{ headers: string[], rows: Record< header, string >[] }`), `list` (`{ items: string[] }`), `label` (`{ pairs: [{ label, value }] }`), `images` (`{ urls: string[] }` — HTTPS only), or `text` (`{ text: string }`). Use slides to attach data tables, bullet lists, key/value records, inline images, or extra text blocks to a card send. Invalid slides are dropped silently (never fail the send). On v2 / unconfigured v3 the slides are ignored.",
    "- Cliq structured-input forms (solicit structured input): `action=send` accepts a `form` param (`{ title?, fields: [{ name, label?, type?: \"select\"|\"text\"|\"number\", options?, placeholder? }] }`) to render a native Cliq prompt card with a button per select option instead of asking for free text. Each `select` field with ≥2 options becomes a `prompt` card (up to 5 buttons; extras listed in the body); `text`/`number` fields fold into a summary card. Tapping a button posts `<fieldName>: <value>` back as a message the agent reads as the answer. Use this to offer fixed choices (priority, environment, yes/no) rather than parsing free-text replies.",
  ];
}
