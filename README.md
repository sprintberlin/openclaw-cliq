...
<p align="center">
  ...
  reply to DMs and channel @mentions as a native bot, with block-streaming live-edit,
...
- **🔁 Reliability** — Durable-before-ack ingest, de-dup on redelivery, bot-loop / self-message protection, outbound retry with error classification (parses the v3 `{"message":"…"}` error envelope). The generated Deluge handler mints a per-execution `eventId`, so a real Cliq `message.id` or that `eventId` retains 30-minute replay protection while two deliberate identical commands (`/new` twice) stay independent turns. Legacy handler scripts that forward no `eventId` fall back to a content-derived id that expires after 60 seconds. Opt-in **bounded inbound catch-up** can recover a direct-chat message that exists in Cliq but never became a webhook turn, using the existing `Messages.READ` endpoint on the next admitted DM — no polling loop or Cliq MCP tools.
...
- OpenClaw `2026.8.2` or later. The plugin is typechecked and built against the pinned minimum version (`2026.8.2`), then that exact built artifact is loaded by a real gateway smoke test on every supported version. CI also checks every static runtime SDK import against each version's published export table.
...
3. Make sure the gateway host is reachable from the public internet (Zoho's servers POST to it). See **[Expose the webhook publicly](https://github.com/sprintberlin/openclaw-cliq/blob/main/docs/setup/public-webhook.md)** for deployment options (VPS + reverse proxy, domain-free sslip.io/nip.io + Caddy, Cloudflare Tunnel, existing proxy, dev tunnels, self-hosted tunnel) with secure Caddy and nginx examples.
   ...
   When the checked URL matches the configured `channels.cliq.publicWebhookUrl`, a passing run records `channels.cliq.inboundVerifiedAt` and a proven failing run records `inboundVerificationFailedAt` (clearing any stale verification) — so verifying via the CLI counts just like verifying via the wizard. The method/reachability boundary retries transient `502`, `503`, `504`, connection-refused/reset, and timeout outcomes three times with 250 ms then 500 ms backoff. Exhausted transient startup conditions remain inconclusive: human and JSON evidence show the bounded attempt count and delay, the command exits non-zero, and both existing verification timestamps are preserved. The write never happens for a URL that is not this install's, and `--no-write` keeps the command a pure read-only probe:
...
- **Channel posts** (`/channelsbyname/{unique_name}/message`, scope `ZohoCliq.Channels.UPDATE`) and **message edits** (`/chats/{chatId}/messages/{messageId}`, scope `ZohoCliq.Messages.UPDATE`) → a **user-context refresh token** obtained once via the self-client `authorization_code` flow. Native v3 typing (`/api/v3/chats/{chat_id}/activities`, scope `ZohoCliq.Chats.UPDATE`) is consented on the same Self Client refresh-token string. The plugin mints short-lived access tokens from the refresh token via `grant_type=refresh_token` and caches them until they expire (~1h). Without a refresh token, channel replies and block-streaming live-edit will fail with `oauthtoken_scope_invalid` — DM-only setups keep working. `client_credentials` can mint a token that reports `Chats.UPDATE`, and a live typing call has returned 204 with that grant, but operators must still add it to the Self Client string and regenerate the user-context refresh token used by the runtime profile.
...
```jsonc
{
  "channels": {
    "cliq": {
      ...
      "streaming": { "mode": "partial" }, // default; set "off" for a single final reply
      "thinking": {
        "mode": "placeholder",
        "animate": "dots"
      }
...
The three sensitive fields accept **three interchangeable representations**, at the channel root and inside `channels.cliq.accounts.<id>`. All three are accepted identically on every supported OpenClaw version (`2026.8.2` and later) — there is no version-gated fallback:
...
- **`ownSenderIds`** *(optional)* — Extra identities that belong to **this** bot, most importantly Cliq's internal `b-…` id. Channel posts from a bot arrive with that internal id, not the unique name. Put those values here so they count as self **and** as this bot in mention/reply directedness checks. Do **not** put other workspace bots here.
- **`selfSenderIds`** *(optional)* — Extra sender ids / names / emails whose inbound messages are dropped as "self" before dispatch. Use this for **other** workspace bots (bot-to-bot loop prevention) and, if needed, a zuid that is not already listed in `ownSenderIds`. A listed bot is still dispatched when it explicitly @mentions this bot. A mention of or reply to a listed *other* bot is **not** treated as directed at this bot ([#285](https://github.com/sprintberlin/openclaw-cliq/issues/285)).
...
- **`inboundCatchup`** *(optional, default off)* — A bounded safety net for a **direct-message** webhook delivery that was missed even though the message exists in Cliq. With `{ "enabled": true, "limit": 50 }`, the next admitted DM fetches at most 50 recent records from `GET /api/v2/chats/{chatId}/messages`, first verifies the current live message as a same-sender/same-text anchor, then dispatches only native-id records strictly between the persisted opaque per-conversation cursor and that anchor, oldest first. First enablement only stores the current anchor — it **never** imports an unbounded existing chat backlog. A missing cursor, absent anchor, failed read, missing refresh token, group/channel turn, malformed history record, self/bot record, or existing dedupe claim yields no recovery and never breaks the live inbound. The cursor is a mode-600 local state record keyed by SHA-256 of account/chat ids; it contains no message content. Requires a user-context `refreshToken` with `ZohoCliq.Messages.READ`; Doctor reports enabled catch-up as unavailable when that credential is absent. Catch-up is event-triggered only — it neither schedules polling nor enables the Cliq MCP/agent message-read action. See [Chat-history recovery](https://github.com/sprintberlin/openclaw-cliq/blob/main/docs/setup/chat-history-recovery.md).
...
- **`groups`** *(optional)* — Per-channel config keyed by the Cliq **channel unique name**. Setup resolves entered channel names through the existing directory adapter. Each entry supports `requireMention` (boolean — `false` lets the bot respond in that channel without an explicit @mention; that always-on path also needs a provisioned `participation_handler` — see [learning 156](docs/learnings/156-channel-inbound-requires-participation-handler-and-mention-optout.md)), `ingest` (boolean), `tools` (`{ allow, alsoAllow, deny }` tool policy for that channel), and `toolsBySender` (per-sender tool policy overrides keyed by `channel:cliq:<senderId>`, `id:<senderId>`, `name:<display>`, `e164:<phone>`, `username:<handle>`, or `*`). A `*` entry applies to any channel not listed explicitly. On an always-on channel the plugin does **not** post a thinking placeholder for unmentioned chatter: if the model answers with Core's exact `NO_REPLY` token, the turn stays completely silent (no `⚠️ Couldn't process that message.` bubble). Directed `@mentions`, replies to the bot, and DMs keep the placeholder. List this bot's internal `b-…` id in `ownSenderIds` and other workspace bots in `selfSenderIds` so their unmentioned posts never start a ping-pong loop ([#283](https://github.com/sprintberlin/openclaw-cliq/issues/283), [#285](https://github.com/sprintberlin/openclaw-cliq/issues/285)). A mention of another bot is not a mention of this bot.
...
- **`thinking`** *(optional)* — Instant acknowledgement / "thinking" placeholder.

  **What you see.** New installs post an animated placeholder by default (`thinking.mode: "placeholder"`, `thinking.animate: "dots"` — cycles `💭 .` → `💭 ..` → `💭 ...` while the agent works). That bubble is the same message the reply later edits in place — not a second progress surface. A "missing" reply is often that bubble still showing `💭 …` because the Cliq client has not redrawn the edit.

  **Turn it off.** Set `thinking.mode: "off"` so no placeholder is posted. To also get a fresh final message instead of an in-place replace of that bubble, set `streaming.mode: "off"`.

  **Modes.** `thinking.mode` selects the acknowledgement style: `"placeholder"` (default — post a lightweight text placeholder, default `💭 …`, configurable via `thinking.text`, then edit it in place into the final reply — exactly one message, no duplicate), `"card"` (post a v3 Message Card status indicator — a `modern-inline` card — instead of plain text, and transition its title through explicit phases as the turn runs: it is first posted with the "thinking" phase title `thinking.thinkingText`, default `💭 thinking…`, then edited in place to the "generating" phase title `thinking.text`, default `Generating…`, right before the agent turn dispatches, and finally edited into the reply text when the reply arrives), or `"off"` (no extra API call per turn). On `apiVersion: "v3"` the card is a real card posted via the v3 Message Card endpoints (DM via `POST /api/v3/bots/{botId}/messages` with scope `Webhooks.CREATE`, channel via `POST /api/v3/channels/{name}/message` with scope `Channels.CREATE`); on v2 it degrades to the plain-text placeholder since v2 has no buttonless card. Native v3 typing (`POST /api/v3/chats/{chat_id}/activities`) is separate and posts as the refresh-token owner, not as the bot; this bot-owned placeholder remains the safe visible progress surface for shared rooms.

  **Animation.** `thinking.animate` cycles the placeholder through text frames on an interval while the agent turn has not produced a real draft update: `"dots"` (default — `💭 .` → `💭 ..` → `💭 ...`), `"spinner"` (braille-spinner frames prefixed with a fixed `thinking…` label), `"custom"` (cycles `thinking.animateFrames`, a string array — needs ≥2 non-empty entries or it degrades to the static placeholder), or `"off"` (static placeholder). The interval is `thinking.animateIntervalMs` (default 1200 ms, **hard-floored to 800 ms** to protect the Cliq edit rate limit) and the total animation duration is capped (default 60 s — past the cap the animation stops and holds the last frame so a long turn does not hammer the edit endpoint). The animation reuses the existing `editMessage` path (`Messages.UPDATE`, same `refreshToken` precondition as the placeholder itself); a failed frame edit stops the animation but never breaks the turn. With `streaming.mode: "partial"`, `"block"`, or `"progress"`, animation covers the otherwise silent tool/reasoning phase, then stops permanently at the first substantive preview/progress/final update. The stop drains an in-flight frame PUT before that real update is applied, so a late frame cannot overwrite the growing draft. No new OAuth scope.

  **Confirmation gate** (card-mode only): set `thinking.confirm` to gate sensitive inbound actions behind an explicit Confirm / Cancel button card instead of dispatching immediately. `"off"` (default) disables gating; `"sensitive"` gates only when the cleaned message matches a `thinking.confirmKeywords` entry (case-insensitive word-boundary match; defaults to a conservative destructive-verb list — `delete`, `drop`, `reset`, `wipe`, `purge`, …); `"always"` gates every turn (apart from abort intents and Confirm-button re-dispatches). When gated, a `prompt`-theme Message Card titled `thinking.confirmText` (default `⚠️ Confirm action?`) with `thinking.confirmLabel` / `thinking.cancelLabel` buttons (defaults `Confirm` / `Cancel`) is posted and the agent turn is held until the user taps a button. **Confirm** re-posts the original message (prefixed with a sentinel) so the next webhook call dispatches the agent with the gate skipped (no re-prompt loop); **Cancel** posts a sentinel that short-circuits the turn with `thinking.cancelledText` (default `🚫 Cancelled.`) and no agent dispatch. The button clicks arrive as ordinary inbound messages via the bot's Message handler (`invoke.bot`) — no Cliq Context handler is required. Messages longer than 1500 chars bypass the gate (cannot be safely encoded in the confirm button payload). The gate is a UX guardrail, not a security boundary — the agent's own tool / permission policy still applies to the confirmed action. No new OAuth scope (reuses the card-path scopes).

  **Requirements.** Any acknowledgement mode is a no-op when no `refreshToken` is configured (editing the placeholder into the reply needs the user-context token). `thinking.failureText` (optional) overrides what the placeholder/card becomes when a **directed** agent turn ends with **no reply** (the turn threw, or the model produced no output): when set, the placeholder is edited into that text (e.g. `⚠️ No reply generated.`); when unset, the default is `⚠️ Couldn't process that message.` Unmentioned always-on channel turns (`requireMention: false` + `participation_handler`) never post this placeholder in the first place, so a Core `NO_REPLY` silent turn cannot leak a failure bubble into the room ([#283](https://github.com/sprintberlin/openclaw-cliq/issues/283)).

- **`streaming`** *(optional)* — Block-streaming live-edit of one Cliq response. This is **not** Telegram-style token streaming.

  **What you see on a default install.** `streaming.mode` defaults to `"partial"` when unset. Existing legacy `streaming.preview: "on"` also resolves to `"partial"`; `preview: "off"` resolves to `"off"`. The thinking placeholder, when present, is the same message the final reply edits in place. A 2026-08-29 production DM (Mara, defaults, `refreshToken` present — issue [#195](https://github.com/sprintberlin/openclaw-cliq/issues/195)) posted a 4-character placeholder (`textLen=4`), sat through ~30s of model + tool work with no intermediate PUTs, then applied one final edit (`textLen=117`, HTTP 204). #175 and #185 never documented a live Cliq DM round-trip; that Mara miss is the first production evidence.
  ...
  **Two switches, not one.** OpenClaw block streaming, Cliq one-message live-edit, and thinking placeholder animation are different layers:
  ...
  | `channels.cliq.streaming.mode` | Cliq one-message live-edit | Selects `"off"`, `"partial"`, `"block"`, or `"progress"`. The current default is `"partial"`; any non-`"off"` mode asks Core for block/partial delivery on that Cliq turn even if the global default is off. `"progress"` is an explicit opt-in: Core tool/item/plan events update one compositor-owned draft, and `onPartialReply` is not used for that mode. |
  | `thinking.animate` | thinking placeholder animation | Cycles placeholder frames during the silent phase; streaming/progress takes ownership at its first substantive draft update. |
  ...
  **HTTP 204 is API acceptance only.** Zoho can accept the final `PUT /api/v2/chats/{chatId}/messages/{messageId}` while the Cliq desktop/app still shows `⏳ …` until the chat is reopened. Native v3 typing is different: a second-account observation proved that accepted activity is visible but attributed to the human refresh-token owner. That is why `heartbeat.typing` defaults to DMs and shared-room progress uses bot-owned editable messages. Live-edit still needs the redraw caveat.

  **Opt out.** Set `streaming.mode: "off"` for a single final reply instead of an in-place replace. Combine with `thinking.mode: "off"` if you do not want the thinking bubble at all. The legacy `streaming.preview: "off"` remains accepted until migrated by doctor.

  **Mechanics.** The thinking animator (`thinking.animate`) may update the shared placeholder while preview/progress is still silent. Before the first substantive draft update, the inbound path stops the timer and drains any frame edit already in flight; every later edit belongs only to live preview/progress, so animation cannot clobber generated text. Live-edit needs `refreshToken` (`ZohoCliq.Messages.UPDATE`); without it, previews degrade to one normal final response and `openclaw cliq doctor` says so. Edits are throttled and coalesced (`streaming.minEditIntervalMs`, default 1000) so unchanged or older content is never sent, and a 429 backs off before retrying the latest preview. Edit failures fall back to one normal final response without failing the turn. Message edit stays on the v2 `PUT /api/v2/chats/{chatId}/messages/{messageId}` path (`ZohoCliq.Messages.UPDATE`); v3 has no single-message edit endpoint.
...
- **`parent` / `quoted` / `parent_message` / `quoted_message` / `reply_to_message`** (object at the payload root, or under `message`, including through the `params` wrapper) — the full parent message `{ id, text, sender: { id, name } }` or a parent id. When the handler forwards this, the agent sees the quoted text + sender directly.

When only the parent **id** is present (e.g. the default `message.reply_to` shape) and a user-context `refreshToken` is configured, the plugin best-effort fetches the parent message text via `GET /api/v2/chats/{chatId}/messages` and prepends it to the agent envelope as:
...
> **Live handler note:** The standard generated Deluge handler currently forwards only the proven `message` string (not an object), so a reply's parent id reaches the plugin only if Deluge forwards `reply_to` / `parent` or if a future Zoho handler exposes the field. Forwarding the parent object is **optional**. Without it the plugin still carries the parent message id whenever one is present; it only cannot show the quoted text unless a `refreshToken` is configured (so the plugin can fetch it). If your Deluge handler can resolve the parent message (e.g. via the Cliq REST `GET /chats/{CHAT_ID}/messages/{MESSAGE_ID}` endpoint), add it under `parent` (or `quoted`) so the agent sees the quote even in DM-only setups with no `refreshToken`. Never invent unproven Deluge variables in the handler script, as unsupported symbols fail permanently with `execution_handler_update_failed`.
...
`openclaw cliq doctor` is the staged diagnostic for a configured Cliq account. Default mode is **read-only**: it never sends a message, never updates a handler, never writes config, and never restarts a service. It reuses the existing doctor warnings, OAuth capability matrix, public webhook preflight, and directory listing instead of reimplementing those checks. When `channels.cliq.publicWebhookUrl` is missing, the public-webhook stage still inspects both Zoho handlers. If they agree on one valid HTTPS `/cliq/webhook` URL and their secret fingerprints match the configured `webhookSecret`, the report names that candidate and the `--adopt-handler-url` repair; it does not guess among differing, missing, or unparseable URLs. `--adopt-handler-url` is the explicit write path: it runs a full authenticated preflight against the candidate, then stores that URL and `inboundVerifiedAt` atomically. A failed or inconclusive preflight, a secret mismatch, or a write error leaves config unchanged. The doctor never updates Zoho handlers.
