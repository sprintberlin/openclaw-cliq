...
- **`thinking`** *(optional)* — Instant acknowledgement / "thinking" placeholder.
  ...
  **Requirements.** Any acknowledgement mode is a no-op when no `refreshToken` is configured (editing the placeholder into the reply needs the user-context token). `thinking.failureText` (optional) overrides what the placeholder/card becomes when the agent turn ends with **no reply** (the turn threw, or the model produced no output): when set, the placeholder is edited into that text (e.g. `⚠️ No reply generated.`); when unset, the untouched placeholder is **deleted** so no stray indicator lingers.
...
// Content-Type and may serialize disposition parameters without double quotes;
// the gateway recognizes this bounded canonical `payload` part from the body
// before parsing the attached FILE parts.
