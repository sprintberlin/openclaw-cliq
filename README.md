...
- **`streaming`** *(optional)* — Block-streaming live-edit of one Cliq response. This is **not** Telegram-style token streaming.
  ...
  **Two switches, not one.** OpenClaw block streaming, Cliq one-message live-edit, and thinking placeholder animation are different layers:
  ...
  | `channels.cliq.streaming.mode` | Cliq one-message live-edit | Selects `"off"`, `"partial"`, `"block"`, or `"progress"`. The current default is `"partial"`; any non-`"off"` mode asks Core for block/partial delivery on that Cliq turn even if the global default is off. `"progress"` is an explicit opt-in while compositor integration is delivered in issues #208–#210. |
