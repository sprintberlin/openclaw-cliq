...
- OpenClaw `2026.8.1-beta.3` or later. The plugin is typechecked and built against the pinned minimum version (`2026.8.1-beta.3`), then that exact built artifact is loaded by a real gateway smoke test on every supported version. CI also checks every static runtime SDK import against each version's published export table.
...
- **`thinking`** *(optional)* — Instant acknowledgement / "thinking" placeholder.
  ...
  **Animation.** `thinking.animate` cycles the placeholder through text frames on an interval while the agent turn runs: `"dots"` (default — `💭 .` → `💭 ..` → `💭 ...`), `"spinner"` (braille-spinner frames prefixed with a fixed `thinking…` label), `"custom"` (cycles `thinking.animateFrames`, a string array — needs ≥2 non-empty entries or it degrades to the static placeholder), or `"off"` (static placeholder). The interval is `thinking.animateIntervalMs` (default 1200 ms, **hard-floored to 800 ms** to protect the Cliq edit rate limit) and the total animation duration is capped (default 60 s — past the cap the animation stops and holds the last frame so a long turn does not hammer the edit endpoint). The animation reuses the existing `editMessage` path (`Messages.UPDATE`, same `refreshToken` precondition as the placeholder itself); a failed frame edit stops the animation but never breaks the turn (the reply is still delivered). Only one animation runs per in-flight message — it is stopped the moment the reply (or failure text) arrives so a late frame edit can never clobber the reply. When `streaming.mode` is not `"off"` (default: `"partial"`), the animator is skipped so frame edits cannot overwrite the live-edit draft; it still runs when `streaming.mode` is `"off"`. No new OAuth scope.
...
Then install. On the supported runtime (**OpenClaw `2026.8.1-beta.3` and later**) a local path is outside ClawHub review and trust metadata, so a non-interactive install is cancelled unless you pass `--force`. That flag is the documented acknowledgement of the warning, not a way to skip a real safety check.
