...
- **`ZohoCliq.Chats.UPDATE`** *(refresh token)* — Native v3 typing via `POST /api/v3/chats/{chat_id}/activities` with body `{"action":"typing"}` (success is empty HTTP 204). A user id is not a chat id. This scope does not unlock `GET /api/v2/chats`. HTTP 204 means the API accepted the call; whether the Cliq client renders a bot typing indicator is unconfirmed.
...
- **`streaming`** *(optional)* — Block-streaming live-edit of one Cliq response. This is **not** Telegram-style token streaming.
  ...
  **HTTP 204 is API acceptance only.** Zoho can accept the final `PUT /api/v2/chats/{chatId}/messages/{messageId}` while the Cliq desktop/app still shows `⏳ …` until the chat is reopened. Native v3 typing already documents the same honesty (`heartbeat.sendTyping` posts `{"action":"typing"}` to `/api/v3/chats/{chat_id}/activities` when a refresh token and a real inbound chat id are present; HTTP 204 is API acceptance only — Cliq client UI visibility is unconfirmed). Live-edit needs that same caveat.
